...
- **Verify:** Confirm the plugin is loaded by checking that `catalog-discovery` and `cdgc-ai-ready-data-assessemt` appear in Claude Code's Skills list and that `cdgc-catalog-discovery` appears in the active MCP servers.
...
- **Uninstall:** `/plugin uninstall informatica-data-discovery@claude-plugins-official` and then restart.
