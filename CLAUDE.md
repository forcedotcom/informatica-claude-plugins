...
- `skills/catalog-discovery/SKILL.md` and `skills/cdgc-ai-ready-data-assessemt/SKILL.md` are real, substantive skills (not placeholders): catalog-first search/hierarchy discovery workflow with certification/trust ranking rules, and a ten-measure CDGC AI-Ready Data Assessment that renders a self-contained HTML scorecard to `artifacts/` (not committed) using `references/*.md` for query/formula/status logic.
