...
description: "Discover, assess, and govern data assets from the CDGC catalog — tables, columns, files, reports, glossary terms, domains, policies — evaluating trustworthiness, business meaning, ownership, sensitivity, applicable policy, and safe use, grounded exclusively in catalog metadata. Use whenever the user asks a data or metadata question. Triggers on: (1) data requests — get me quarterly sales for EMEA, pull revenue by region, how much sales last quarter, I need order data, do we have customer tables, give me [topic] data; (2) discovery — find customer data, where is revenue data, show me sales tables, look up the orders table, what tables have [X], is there a report on [Y]; (3) trust and governance — who owns this data, what does this column mean, is this PII, can I rely on this, is this the authoritative source, what policy applies, is this safe to use for [purpose], is this sensitive / restricted. Always run BEFORE reading actual data through other MCPs so the caller picks the right, trusted, compliant asset. SKIP: writing/editing catalog assets, running ETL or pipelines, querying row data without needing to find the right asset first."
---
